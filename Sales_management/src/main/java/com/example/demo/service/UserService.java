...
public class UserService {
	...
	public Page<ManagementList> getList(Pageable pageable) {
		return (Page<ManagementList>) userRepository.findAll((Specification
				.where(UserSpecifications.delete_flgCheck())), pageable);
	}

	// 一覧取得(検索条件がある場合)
	public Page<ManagementList> getListSerch(Pageable pageable, Long customer_id, int status_id, String serch_subject) {
		return (Page<ManagementList>) userRepository.findAll((Specification
				...
				.and(UserSpecifications.delete_flgCheck())), pageable);
	...
	public List<Customer> getcoustomer_list() {
		return customerRepository.findAll((Specification
				.where(UserSpecifications.customer_delete_flgCheck())));
	}
	...
	// 顧客削除確認
	public Long getListcheck(Long customer_id) {	// 削除する顧客が使われている案件を確認する
		return userRepository.count((Specification
